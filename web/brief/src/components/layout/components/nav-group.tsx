...
export function NavGroup({ title, items }: NavGroupProps) {
  ...
  return (
    <SidebarGroup className='px-0 py-1'>
      {title ? (
        <SidebarGroupLabel className='px-5 text-[11px] font-semibold tracking-[0.16em] text-slate-400 uppercase'>
          {title}
        </SidebarGroupLabel>
      ) : null}
      <SidebarMenu>
  ...
  return (
    <SidebarMenuItem>
      <SidebarMenuButton
        ...
        tooltip={item.title}
        className='h-11 rounded-xl px-5 text-base font-medium text-slate-700 data-active:bg-blue-50 data-active:text-blue-600 hover:bg-slate-50 hover:text-blue-600 [&_svg]:size-5'
    ...
    >
      <CollapsibleTrigger
        ...
        render={
          <SidebarMenuButton
            tooltip={item.title}
            className='h-11 rounded-xl px-5 text-base font-medium text-slate-700 hover:bg-slate-50 hover:text-blue-600 [&_svg]:size-5'
          />
        }
      >
        {item.icon && <item.icon className='shrink-0' />}
          ...
          {item.items.map((subItem) => (
            <SidebarMenuSubItem key={subItem.title}>
              <SidebarMenuSubButton
                ...
                className='h-10 rounded-xl px-14 text-base font-medium text-slate-600 data-active:bg-blue-50 data-active:text-blue-600 hover:bg-slate-50 hover:text-blue-600'
                render={
